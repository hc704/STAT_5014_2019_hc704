...
2. Summary statistics of pH difference of different foliage color groups 
```{r}
library(dplyr)
...
3. Use linear model and ANOVA results to check the relationship between foliage color and ph Difference.
```{r}
lm(pH_Dif ~ Foliage_Color, plants)
```

```{r}
summary(aov(pH_Dif ~ Foliage_Color, plants))
```
